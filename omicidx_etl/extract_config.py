...
class Settings(BaseSettings):
    ...
    Deprecated: Use get_path_provider() instead.
    """
    ...
    def __init__(self, **kwargs):
        if "PUBLISH_DIRECTORY" not in kwargs and not os.getenv("PUBLISH_DIRECTORY"):
            provider = get_path_provider()
            kwargs["PUBLISH_DIRECTORY"] = str(provider.base_dir)
        super().__init__(**kwargs)

    @property
    def publish_directory(self) -> UPath:
        """Legacy property - use get_path_provider().base_dir instead."""
        if self.PUBLISH_DIRECTORY:
            return UPath(self.PUBLISH_DIRECTORY)
        return get_path_provider().base_dir


# Keep legacy settings for gradual migration
settings = Settings()
...
if __name__ == "__main__":
    ...
    print("PathProvider Demo")
    print("=" * 60)

    # Get provider
    provider = get_path_provider()
    print(f"Base directory: {provider.base_dir}\n")

    # Demonstrate path computation
    print("Path computation:")
    print(f"  SRA:              {provider.get_path('sra')}")
    print(f"  SRA studies:      {provider.get_path('sra', 'studies')}")
    print(f"  GEO:              {provider.get_path('geo')}")
    print(f"  BioSample:        {provider.get_path('biosample')}\n")

    # Demonstrate file registration
    print("Registering sample files...")
    provider.register_file(
        "sra",
        provider.get_path("sra", "studies.parquet"),
        {"record_count": 500000, "schema_version": "1.0"}
    )
    provider.register_file(
        "geo",
        provider.get_path("geo", "series.ndjson.gz"),
        {"record_count": 250000}
    )

    # Show registry
    print("\nFile registry:")
    print(json.dumps(provider.export_registry(), indent=2))

    # Show registered files for SRA
    print("\nSRA registered files:")
    for entry in provider.get_registered_files("sra"):
